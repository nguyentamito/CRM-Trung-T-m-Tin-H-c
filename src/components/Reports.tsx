...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isWithinInterval, parseISO, startOfDay, endOfDay, eachMonthOfInterval, startOfYear, endOfYear, subMonths, subDays } from 'date-fns';
...
export default function Reports({ 
  ...
  const [activeReportTab, setActiveReportTab] = useState<'financial' | 'crm' | 'learning' | 'staff_attendance'>('financial');
  ...
  const filteredData = useMemo(() => {
    ...
    const filteredCustomers = customers.filter(c => {
      const cDate = Number(c.createdAt || 0);
    ...
    const filteredReceipts = receipts.filter(r => {
      const rDate = Number(r.date || r.createdAt || 0);
    ...
    const filteredVouchers = paymentVouchers.filter(v => {
      const vDate = Number(v.date || v.createdAt || 0);
    ...
    const filteredAttendance = attendance.filter(a => {
      ...
      const sDate = Number(session.date || 0);
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={cn(
              ...
              activeReportTab === 'financial' ? "bg-[#2D5A4C] text-white shadow-lg shadow-[#2D5A4C]/20" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            )}
          >
            Tài chính
          </button>
          <button
            ...
            className={cn(
              "px-4 py-2 text-sm font-bold rounded-xl transition-all",
              activeReportTab === 'crm' ? "bg-[#2D5A4C] text-white shadow-lg shadow-[#2D5A4C]/20" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            )}
          >
            Kinh doanh
          </button>
          <button
            onClick={() => setActiveReportTab('learning')}
            className={cn(
              "px-4 py-2 text-sm font-bold rounded-xl transition-all",
              activeReportTab === 'learning' ? "bg-[#2D5A4C] text-white shadow-lg shadow-[#2D5A4C]/20" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            )}
          >
            Học tập
          </button>
          <button
            onClick={() => setActiveReportTab('staff_attendance')}
            className={cn(
              "px-4 py-2 text-sm font-bold rounded-xl transition-all",
              activeReportTab === 'staff_attendance' ? "bg-[#2D5A4C] text-white shadow-lg shadow-[#2D5A4C]/20" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            )}
          >
            Chấm công NV
          </button>
        ...
        <>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-8 border-b border-gray-50 flex justify-between items-center">
              ...
              <button 
                onClick={() => {
                  const headers = ['Nhân viên', 'Khách mới', 'Đã chốt', 'Doanh số', 'Thực thu', 'Lịch hẹn'];
                  const csvData = staff.map(s => {
                    ...
                    return [
                      s.displayName,
                      staffCustomers.length,
                      staffClosedDeals.length,
                      staffSales,
                      staffIncome,
                      staffAppointments
                    ].join(',');
                  ...
                  link.setAttribute('download', `hieu-suat-nhan-vien-${dateRange.start}-${dateRange.end}.csv`);
                        ...
                        <td className="px-8 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-[#2D5A4C]/10 flex items-center justify-center text-[#2D5A4C] font-bold text-xs">
                              {s.displayName && s.displayName !== '0' ? s.displayName.charAt(0) : '?'}
                            </div>
                            <span className="font-bold text-gray-900">
                              {s.displayName === '0' || !s.displayName ? 'Chưa đặt tên' : s.displayName}
                            </span>
                          </div>
                        </td>
